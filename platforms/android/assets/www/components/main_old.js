var server = "http://192.168.1.203/jayaam/public/";
$(function() {
    ...
    $(document).on('click', '.approve', function(event) {
        if (confirm("Are you sure you want to approve?")) {
            ...
            $.post(server + 'processapproval', data, function(result) {
                alert(result);
                 $(this).closest('.ons-list-item-inner').hide();
            });
            return false;
        }
        return false;
    ...
    $(document).on('submit', '#zinctablab', function(event) {
      ...
      $(document).on('submit', '#recoveryloginsert', function(event) {
        ...
        console.log("data: " + data);
        $.post(server + 'recoveryloginsert', data, function(result) {
            alert(result);
        });
        return false;
    });
